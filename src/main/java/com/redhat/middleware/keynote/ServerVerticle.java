...
public class ServerVerticle extends AbstractVerticle {
    ...
    public final static Logger LOGGER = LogManager.getLogger(ServerVerticle.class);
    ...
    public void start(Future<Void> future) throws Exception {
        ...
        vertx.createHttpServer()
                .websocketHandler(ws -> {
                    switch (ws.path()) {
                        ...
                        case "/game":
                            ...
                            break;
                        default:
                            LOGGER.warn("Connection to web socket [ path = {} ] not supported", ws.path());
                            ws.reject();
                    }
                })
                ...
                .listen(config().getInteger("port", 9001), ar -> {
                    if (ar.succeeded()) {
                        LOGGER.info("Game server started on 0.0.0.0:{}", ar.result().actualPort());
